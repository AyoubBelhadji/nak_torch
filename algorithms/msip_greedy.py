...
def msip_map(objective_function, particles, kernel_bandwidth=1.0,bandwidth_factor=0.5, bounds = [0], projection = True, gradient_informed = True):
    """
    ...
    with torch.no_grad():
        ...
        # You can remove these prints once you're happy
        # print(kernel_matrix)
        ...
        for i in range(N):
            ...
            if gradient_informed == True:
                ...
                if projection == False:
                    ...
                    t_list.append(torch.clamp(t1, min=lower_bounds, max=upper_bounds))
        #print(i)
...
def update_one_particle(objective_function,
    ...
    for _ in range(max_inner_steps):
        ...
        if move_norm.isnan() ==True:
            print('nan')
        if move_norm.item() < inner_tol:
            #print(_)
            # Approximate MSIP equilibrium for this particle
            break
        # if _ == max_inner_steps-1:
        #     print(_)

    return new_list_particles


def msip_greedy(objective_function, 
         n_particles=50,
