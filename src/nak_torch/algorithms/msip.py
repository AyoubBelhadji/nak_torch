...
def update_particles_(objective_function, particles, t, lr=0.1, noise=0.05, kernel_bandwidth=1.0):
    ...
    f_times_y = fitness.unsqueeze(-1) * particles
    ...
    with torch.no_grad():
        ...
        particles = (1 - lr) * particles + lr * t_arr
...
def update_particles(objective_function, particles, t, lr=0.1, noise=0.05, kernel_bandwidth=1.0, bandwidth_factor=0.5, bounds = [0], projection = True):
    # Make sure this is a leaf with grad
    particles = particles.detach().clone()
    particles.requires_grad_(True)
    ...
    lower_bounds = torch.as_tensor(bounds[0], dtype=particles[0].dtype, device=particles[0].device)
    upper_bounds = torch.as_tensor(bounds[1], dtype=particles[0].dtype, device=particles[0].device)
    ...
    with torch.no_grad():
        ...
        for i in range(N):
            ...
            if projection == False:
                t_list.append(t1 + bandwidth_factor*bandwidth_factor*sigma2 * t2)
            else:
                t_list.append(torch.clamp(t1 + bandwidth_factor*bandwidth_factor*sigma2 * t2, min=lower_bounds, max=upper_bounds))
...
def msip(objective_function, 
    ...
    for _ in range(n_steps):
        particles = update_particles(objective_function, particles, 0,lr=lr, noise=noise, kernel_bandwidth= kernel_bandwidth)
