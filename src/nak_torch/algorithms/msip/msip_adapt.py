...
from nak_torch.tools.types import BatchPtType, LogDensity, BatchLogDensity, \
...
def msip_adapt(
    ...
    **msip_kwargs
    ...
    if choose_running is None:
        def _choose_running(_: BatchPtType, running: BatchType): return running
    ...
    if keep_all:
        trajectories = torch.empty(
            (n_steps+1, *particles.shape),
            dtype=particles.dtype, device=device
        )
    ...
    for idx in tqdm(range(n_steps), disable=not verbose):
        ...
        with torch.no_grad():
            dt, t, state, particles, accept = step_fcn(
                state, running, dt,
                t, particles, (kernel_length_scale,)
            )
