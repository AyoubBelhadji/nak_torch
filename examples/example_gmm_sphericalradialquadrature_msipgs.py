...
def mc_quad_rule(batch_size: int, N_quad: int = 15, dim: int = 2):
...
traj_gi_sph, wts_gi_sph = msip_gs(
...
msip_gf_mc = MSIPQuadGradientFree(
    post_log_dens_batch,
    partial(mc_quad_rule, N_quad=15),
)
traj_gf_mc, wts_gf_mc = msip_gs(
...
traj_gf_sph, wts_gf_sph = msip_gs(
...
results = [
    ...
    (traj_gi_sph[-1], wts_gi_sph[-1], "GradInformed + SRQ"),
    (traj_gf_mc[-1],  wts_gf_mc[-1],  "GradFree + MC"),
    (traj_gf_sph[-1], wts_gf_sph[-1], "GradFree + SRQ"),
]
