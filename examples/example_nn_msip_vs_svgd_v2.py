...
class sigma_pi(nn.Module):
    """
    Sigma-Pi network rewritten with einsum (same architecture as original,
    faster than nested loops).

    output = Σ_n w_n · Π_m (α_m + β_m · act(x @ v_m + bias_m))
    """
    def __init__(self, d, M, N, C, activation='ReLU'):
        super().__init__()
        ...
        self.d, self.C, self.M, self.N = d, C, M, N

    def forward(self, xb):
        # xb   : (B, d)
        # proj : (B, M, N, C)
        proj = torch.einsum('bd,dmnc->bmnc', xb, self.vs) + self.bias
        ...
        gate = self.alphas + self.betas * act   # (B, M, N, C)
        prod = gate.prod(dim=1)                  # (B, N, C)
        out  = torch.einsum('bnc,nc->bc', prod, self.ws)  # (B, C)
        return out
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 2 — DATASET LOADING WITH TRAIN / TEST SPLIT
# ══════════════════════════════════════════════════════════════════════════════
...
def make_objective(X, Y, model_class='bnn',
    ...
    if model_class == 'sigma_pi':
        model = sigma_pi(d, M=1, N=hidden_dim, C=1, activation='ReLU').double()
    elif model_class == 'bnn':
...
def plot_boundary_fan(trajectories_dict, objective_fns_dict,
...
def plot_mutual_information(trajectories_dict, objective_fns_dict,
                             X_tr, Y_tr, bounds=[-0.1, 1.1], M_res=60):
    Xg, Yg, grid = _make_grid(bounds, M_res)
    M = len(trajectories_dict)
    fig, axes = plt.subplots(1, M, figsize=(6*M, 5))
    ...
    for ax, (name, traj) in zip(axes, trajectories_dict.items()):
        probs = _get_ensemble_probs(traj, objective_fns_dict[name], grid)
        p_bar = probs.mean(0)
        MI    = (_bH(p_bar) - _bH(probs).mean(0)).reshape(M_res, M_res)
        im = ax.imshow(MI, extent=[*bounds, *bounds], origin='lower',
                       cmap='viridis', vmin=0)
        plt.colorbar(im, ax=ax, label='MI (nats)')
        ax.contour(Xg, Yg, p_bar.reshape(M_res, M_res),
                   levels=[0.5], colors=['white'], linewidths=1.5)
        _scatter(ax, X_tr, Y_tr)
        ax.set_title(f"{name}  —  mean MI = {MI.mean():.3f}",
...
#     """Accuracy + confidence + mean MI on a (X, Y) tensor pair."""
#     y_true = (Y.numpy() > 0)
#     grid   = X.double()
...
#     print(f"{'─'*65}")
#     print(f"  {'Method':<12} {'Acc':>6} {'Confidence':>12} {'Mean MI':>10}")
#     print(f"{'─'*65}")
...
#         mi         = (_bH(p_bar) - _bH(probs).mean(0)).mean()
#         print(f"  {name:<12} {acc:>6.3f} {confidence:>12.3f} {mi:>10.3f}")
#         results[name] = dict(p_bar=p_bar, accuracy=acc,
#                              confidence=confidence, mean_mi=mi)
...
def evaluate(trajectories_dict, objective_fns_dict, X, Y, split_name="test"):
    ...
    for name, traj in trajectories_dict.items():
        ...
        results[name] = dict(p_bar=p_bar, accuracy=acc,
                             confidence=confidence, mean_mi=mi, damv=damv)

    print(f"{'─'*75}\n")
    return results

# ══════════════════════════════════════════════════════════════════════════════
# SECTION 8 — LEGACY DIAGNOSTICS (unchanged)
# ══════════════════════════════════════════════════════════════════════════════
...
if __name__ == "__main__":
    ...
    # ── Config ────────────────────────────────────────────────────────────────
    DATASET      = 'two_bananas'
    MODEL_CLASS  = 'bnn'      # ← swap to 'sigma_pi' to use the other model
    HIDDEN_DIM   = 5         # hidden layer width  (both models)
    N_LAYERS     = 1          # depth  (bnn only)
    ...
    # ── Legacy diagnostics (MSIP only) ────────────────────────────────────────
    eval_tensor = eval_function_trajectories(obj_msip, trajectories_msip)
    plot_eval_tensor(eval_tensor)
    plot_eval_best_tensor(eval_tensor)

    # ── Comparison plots ──────────────────────────────────────────────────────
    plot_boundary_fan(trajectories_dict, objective_fns_dict, X_train, Y_train)
    #plot_mutual_information(trajectories_dict, objective_fns_dict, X_train, Y_train)
