...
def forward_particles(
    ...
    # Output
    mu = torch.einsum("pbh,phq->pbq", hidden, W2).squeeze(-1) + b2[:, None, :]
    mu = mu.squeeze(-1)  # [P, B]
...
def minibatch_log_posterior(theta, x_batch, y_batch, n_train, shape, cfg):
    ...
    y = y_batch.view(1, B)                                    # [1, B] — guaranteed 1D first
    mu = mu.view(P, B)                                        # [P, B] — guaranteed 2D
...
def minibatch_log_posteriorX(theta, x_batch, y_batch, n_train, shape, cfg):
    ...
    loglik = -0.5 * (
        residual2 / sigma2 +
        torch.log(2 * torch.pi * sigma2)
    )                                                           # [P,B]

    loglik_per_particle = loglik.sum(1)                         # [P]

    scale = n_train / B
    loglik_per_particle = scale * loglik_per_particle           # [P]
